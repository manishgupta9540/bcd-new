...
use App\Events\CandidateApiCheck;
...
class CandidateCheckListener
{
     ...
     * Handle the event.
     *
     * @param  CandidateApiCheck  $event
     * @return void
     */
    public function handle(CandidateApiCheck $event)
    {
        $super_parent_id=0;
        $parent_id=Auth::guard('candidate')->user()->parent_id;
        $business_id=Auth::guard('candidate')->user()->business_id;
        $user_id=Auth::guard('candidate')->user()->id;
      
        $user_type=Auth::guard('candidate')->user()->user_type;
        ...
        if (isset($event->jfd_service) && count($event->jfd_service)) {
            foreach ($event->jfd_service as $service) {
              ...
              if($service->is_insufficiency==0)
              {
               ...
               if($user_type=='candidate')
                {
                  $data = DB::table('check_price_cocs')
                          ->where(['service_id'=>$service->service_id,'coc_id'=>$business_id])
                          ->first();
                    //dd($data);
                  if($data!=NULL)
                  {
                            ...
                            foreach($kams as $kam)
                            {
                                ...
                                //get Mail config data
                                  ...
                                  if (count($mail)>0) {
                                      ...
                                      //     $message->from(env('MAIL_FROM_ADDRESS'),env('MAIL_FROM_NAME'));
                                      // });
                                      dispatch(new \App\Jobs\Admin\Candidate\Jaf\InsuffClearNotifyConfig($data,$email1,$name1));
                                  }
