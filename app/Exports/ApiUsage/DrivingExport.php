...
class DrivingExport implements FromCollection, WithHeadings, ShouldAutoSize, WithEvents, WithMapping
{
    use Exportable;
    ...
    */
    ...
    {
        $new_arr=[$data->dl_number,ucfirst($data->name),Helper::user_name($data->user_id),date('d-F-Y h:i:s',strtotime($data->created_at)),'₹ '.$data->price];
