        ...
        <table class="table table-bordered customerTable">
            <thead class="thead-light">
                <tr>
                    ...
                    <th scope="col" style="position:sticky; top:60px">Video Link</th>
                ...
                @if(count($items)>0)
                    @foreach ($items as $key => $item)
                        <tr>
                            ...
                            <td>         
                                <a href="javascript:void(0)"> <button class="btn btn-outline-info btn-md mb-1 editinvideolinkbtn" data-id="{{ base64_encode($item->id) }}" type="button" > <i class="fa fa-edit"> </i> Edit</button> </a>   
                                @if($item->status==1)
...
<div class="modal" id="edit_videolink">
    <div class="modal-dialog">
       <div class="modal-content">
          ...
          <div class="modal-header">
              <div class="row">
                <div class="col-11">
                    <h4 class="modal-title">Add Video link client Wise</h4>
                </div>
                <div class="">
          ...
          <!-- Modal body -->
            <form method="post" action="{{url('jafvideo/links/edit')}}" id="insuff_update">
                @csrf
                ...
                <div class="form-group">
                    ...
                    <div class="form-group">
                    <label for="label_name">Add Jaf Video Link :</label>
                    <input type="text" id="video_jaf_links" name="video_jaf_links" class="form-control video_jaf_links" placeholder="Enter Jaf Video Links"/>
                    <p style="margin-bottom: 2px;" class="text-danger error-container error-video_jaf_links" id="error-video_jaf_links"></p> 
                    </div>
                </div>
                <!-- Modal footer -->
 ...
 <script>
     $(document).ready(function(){
        ...
        $('.editinvideolinkbtn').click(function(){
            var id=$(this).attr('data-id');

            $('.form-control').removeClass('is-invalid');
            $('.error-container').html('');
            $('.btn-disable').attr('disabled',false);
            $('#edit_videolink').modal({
                backdrop: 'static',
                keyboard: false
            });
            $.ajax({
                type: 'GET',
                url: "{{ url('/jafvideo/links/edit') }}",
                data: {'id':id},        
                success: function (data) {
                    ...
                    {              
                        ...
                        $('.video_jaf_links').val(data.result.video_jaf_links);
