        ...
        <tbody>
            @if(count($items)>0)
                @foreach ($items as $key => $item)
                    @php
                        $report_share_complete = Helper::getNotificationControlData($item->business_id,'report-share-complete');
                    @endphp
                    <tr>
                        <td>{{Helper::user_name($item->business_id)}} - {{ Helper::company_name($item->business_id)}}</td>
                        <td>{{$item->days}}</td>
                        <td>
                            @if($report_share_complete!=NULL)
                                @if($report_share_complete->status==1)
