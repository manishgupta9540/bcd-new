                  ...
                  <div class="col-md-9 content-wrapper" style="background:#fff">
                     <div class="formCover" style="height: 100vh;">
                        ...
                        <section>
                           ...
                           @include('admin.accounts.notification.report.menu')
                           <div class="col-sm-12 ">
                                 <!-- row -->
                                 <div class="row">
                                    <div class="col-md-9">
                                       ...
                                       <span>
                                          Note:- Add the mail ids with the number of days on which you want to send the report.
                                       </span>
                                       <p class="pb-border"></p>
                                    </div>
                                    <div class="col-md-3 text-right mt-3">
                                      <div class="btn-group" style="float:right">
                                        ...
                                        @endif
                                        <span><a class="btn btn-success add_new" href="javascript:;" > <i class="fa fa-plus"></i> Add New </a></span>
                                       </div>
                                    </div>
                                 </div>
                                 <!-- ./business detail -->
                           ...
                           <div class="row">
                            <div class="col-md-12 pt-3">
                                <div id="candidatesResult">
                                    @include('admin.accounts.notification.report.ajax')        
                                 </div>
                            </div>
                         </div>
                        </section>
...
<div class="modal" id="add_notify_modal">
   <div class="modal-dialog modal-lg">
      <div class="modal-content">
         ...
         <!-- Modal body -->
         <form method="post" action="{{url('/notification/report/default/store')}}" id="notify_add_frm">
         @csrf
               ...
               <div class="row">
                  <div class="col-12">
                     <div class="form-group">
                        ...
                        <option value="">-- Select-- </option>
                           @if(count($notify_customers)>0)
                              @foreach($notify_customers as $customer)
                                 <option value="{{ $customer->id }}"> {{ $customer->company_name.' - '.$customer->first_name}} </option>
                              @endforeach
                           @endif
                        </select>
                  ...
                  <div class="col-12">
                     <div class="form-group">
                           ...
                           <input type="text" name="no_of_days" class="form-control no_of_days" value="1" placeholder="Enter No. of days"/>
                           <p style="margin-bottom: 2px;" class="text-danger error-container error-no_of_days" id="error-no_of_days"></p> 
                     </div>
                  </div>
                  <div class="col-12">
                     <h5 class="text-muted">Details <span class="text-danger">*</span>: You can add additional mail Id by clicking on additional mail Id button, mail will be sent to added Mail Id.</h5>
                     <p class="pb-border"></p>
                     <div class="addDataDiv">
                     </div>
                     <a href="javascript:;" class="add_data"><i class="fa fa-plus my-4"></i> Additional Mail ID</a>
                  </div>
                  <div class="col-12">
...
<div class="modal" id="notify_modal">
    <div class="modal-dialog modal-lg">
       <div class="modal-content">
          ...
          <!-- Modal body -->
          <form method="post" action="{{url('/notification/report/default/edit')}}" id="notify_frm" enctype="multipart/form-data">
          @csrf
            <input type="hidden" name="id" class="id" id="id">
             ...
             <!-- Modal footer -->
             <div class="modal-footer">
                <button type="submit" class="btn btn-info btn-disable submit_btn">Update </button>
                <button type="button" class="btn btn-danger btn-disable" data-dismiss="modal">Close</button>
             </div>
          </form>
       </div>
...
<script>
    $(document).ready(function() {
      ...
      $(document).on('change','.to_date',function() {
       ...
       $('.add_new').click(function(){
         $("#notify_add_frm")[0].reset();
         $('.form-control').removeClass('border-danger');
         $('.error-container').html('');
         $('.btn-disable').attr('disabled',false);
         $('.modal-footer').addClass('d-none');
         $('#add_notify_modal').modal({
                backdrop: 'static',
                keyboard: false
         });
       });

      $(document).on('submit', 'form#notify_add_frm', function (event) {
            event.preventDefault();
                  ...
                  success: function (response) {
                     ...
                     if(response.success==false ) {                              
                           for (control in response.errors) {
                              ...
                              $('#notify_add_frm').find('#error-' + error_text).html(response.errors[control]);
                           }
                     }
                  },
         ...
         swal({
            ...
            function(e){
               if(e==true)
               {
                      $.ajax({
                           type:'POST',
                           url: "{{ url('/')}}"+"/notification/report/default/status",
      ...
      $(document).on('click', '.editBtn', function (event) {
            ...
            $.ajax({
               type: 'GET',
               url: "{{ url('/notification/report/default/edit') }}",
               data: {'id':id},        
      ...
      $(document).on('click','.close_div',function(){
         ...
         _this.parent().parent().parent().parent().fadeOut("slow", function(){ 
               var s_len = _this.parent().parent().parent().parent().parent().find('.cust_data').length;
               if(s_len<=1)
               {
                  $('.modal-footer').addClass('d-none');
               }
               ...
               $.ajax({
                  ...
                  success: function (response) {
                     ...
                     if(response.success==true) {          
                           ...
                           //notify
                           toastr.success("Notification Setting Has Been Updated Successfully !!");
                           // redirect to google after 5 seconds
                           window.setTimeout(function() {
                               window.location.reload();
                     ...
                     if(response.success==false ) {                              
                           for (control in response.errors) {
                              var error_text = control.replace('.',"_");   
                              $('#notify_frm').find('#error-' + error_text).html(response.errors[control]);
                           }
                     }
                  },
                  error: function (response) {
            ...
            swal({
               ...
               function(e){
                  if(e==true)
                  {
                     $.ajax({
                           type:'POST',
                           url: "{{route('/notification/report/default/delete')}}",
    ...
    {
            var i=0;
            $('#notify_add_frm').find('.error-name').each(function(){
               $(this).attr('id','error-name_'+i);
               i++;
            });

            var i=0;
            $('#notify_frm').find('.error-name').each(function(){
               $(this).attr('id','error-name_'+i);
               i++;
            });

            var i=0;
            $('#notify_add_frm').find('.error-email').each(function(){
               $(this).attr('id','error-email_'+i);
               i++;
            });

            var i=0;
            $('#notify_frm').find('.error-email').each(function(){
               $(this).attr('id','error-email_'+i);
               i++;
            });

            var i=0;
            $('#notify_add_frm').find('.sts_r').each(function(){
               $(this).attr('name','status-'+i);
               i++;
            });

            var i=0;
            $('#notify_frm').find('.sts_r').each(function(){
               $(this).attr('name','status-'+i);
               i++;
            });

            var i=0;
            $('#notify_add_frm').find('.error-status').each(function(){
               $(this).attr('id','error-status-'+i);
               i++;
            });

            var i=0;
            $('#notify_frm').find('.error-status').each(function(){
               $(this).attr('id','error-status-'+i);
               i++;
            });
