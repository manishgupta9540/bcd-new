         ...
         <div class="card text-left">
            ...
            <div class="col-md-12 text-center"><h2 class="mb-0">[Restricted]</h2></div>
               ...
               <div class="row">
                  ...
                  <div class="col-md-3 form-group mt-4">
                     <a class="btn-link bulk_assign" id="bulkAssignPopup" href="javascript:;"> <i class="fas fa-user-plus"></i> Bulk Assign </a><i class="fa fa-info-circle tooltips" data-toggle="tooltip" data-original-title="You can assign report generation case and task verification case in bulk from here"></i> 
               ...
               <div id="taskResult">
                  @include('admin.task.all-task.ajax')
      ...
      <!-- Modal Header -->
         ...
         <div class="modal-body">
            ...
            <div class="form-group">
               ...
               <select class="form-control bulk_task_type" name="bulk_task_type" id="bulk_task_type" >
                  <option value="">Select</option>
                  <option  value="jaf">JAF Link</option>
                  <option  value="task">Task for Verification</option>
            ...
            <div class="form-group" id="bulk_users_type">
...
<div class="modal" id="assign_modal">
   <div class="modal-dialog">
      <div class="modal-content">
         ...
         <!-- Modal footer -->
         <div class="modal-footer">
            <button type="submit" class="btn btn-info btn-submit submit_assign_form submit">Submit </button>
...
<script type="text/javascript">
 $('.tool').tooltip();
   ...
   $(document).ready(function(){
      ...
      $(document).on('change','.bulk_task_type',function(){
        
         var bulk_task_type =$('#bulk_task_type option:selected').val();
         if (bulk_task_type=='task') {
            $('#bulk_users_type').html("<input  type='radio' class='bulk_user_status' id='bulk_user_status' name='bulk_user_status' value='user' ><label for='bulk_user_status'> User</label> <input type='radio' class='bulk_user_status' id='bulk_vendor_status' name='bulk_user_status' value='vendor'><label for='bulk_vendor_status'> Vendor</label>").fadeIn(300);

         } else {
            $('#bulk_users_type').html("<input  type='radio' class='bulk_user_status' id='bulk_user_status' name='bulk_user_status' value='user' ><label for='bulk_user_status'> User</label> ").fadeIn(300);
      ...
      $(document).on('change','.bulk_user_status',function(){
         ...
         if (bulk_user_status == 'vendor') {
            ...
            $.ajax({
               ...
               data: {'service_id':assign_service,'task_time':assign_task_time,'created_time':assign_created_time,'user_type':bulk_user_status,'bulk_task_type':bulk_task_type},
                  ...
                  success: function (data) {
                     ...
                     if (data.fail == false ) {
                        ...
                        if (data.status =='ok') {
                           ...
                           toastr.success("Task has not been assigned to any user,Please check the user permissions or task's type!");
                        }
                        // toastr.success("Task has been assigned successfully");
                           // redirect to google after 5 seconds
                           window.setTimeout(function() {
      ...
      // filterBtn
      $(document).on('change','.from_date, .to_date, .candidate_list,.customer_list,.sla_list,.user_list,.ref_list,#rows,#service,#task_type,#assign_status,#complete_status,.ref', function (e){    
         $("#overlay").fadeIn(300);　
         getData(0);
         e.preventDefault();
      });
      ...
      $(document).on('click','.assign_user',function(){
         ...
         $(document).on('submit', 'form#assign_form', function (event) {
            ...
            var url = form.attr("action");
            var loadingText = '<i class="fa fa-circle-o-notch fa-spin"></i> loading...';
            $('.submit_assign_form').attr('disabled',true);
            $('#assign_back').attr('disabled',true);
            $('.form-control').attr('readonly',true);
            $('.form-control').addClass('disabled-link');
            $('.error-control').addClass('disabled-link');
            if ($('.submit_assign_form').html() !== loadingText) {
                $('.submit_assign_form').html(loadingText);
            }
            // _this.find('.submit_assign_form').attr('disabled', true);
            // _this.find('#assign_back').attr('disabled', true);
            ...
            ({
                  ...
                  success: function (response) {
                     window.setTimeout(function(){
                        $('.submit_assign_form').attr('disabled',false);
                        $('#assign_back').attr('disabled',false);
                        $('.form-control').attr('readonly',false);
                        $('.form-control').removeClass('disabled-link');
                        $('.error-control').removeClass('disabled-link');
                        $('.submit_assign_form').html('Submit');
                      },2000);
   ...
   function getData(page){
      ...
      var assign_status = $("#assign_status option:selected").val()==undefined?'':$("#assign_status option:selected").val();
      var complete_status = $("#complete_status option:selected").val()==undefined?'':$("#complete_status option:selected").val();
      ...
      //   var report_status=$('.report_status').val();               
         ...
         $.ajax(
         {
               url: '?page=' + page+'&customer_id='+user_id+'&from_date='+from_date+'&to_date='+to_date+'&candidate_id='+candidate_id+'&sla_id='+sla_id+'&user_id='+cus_user_id+'&rows='+rows+'&service_id='+service_id+'&task_type='+task_type+'&assign_status='+assign_status+'&complete_status='+complete_status+'&ref_list='+selected+'&ref='+ref,
               type: "get",
               datatype: "html",
         })
         .done(function(data)
   ...
   function setData(){
      ...
      // var report_status=$('.report_status').val();
            $.ajax(
            {
               url: "{{ url('/') }}"+'/candidates/setData/?customer_id='+user_id+'&from_date='+from_date+'&to_date='+to_date+'&candidate_id='+candidate_id+'&sla_id='+sla_id+'&user_id='+cus_user_id+'&rows='+rows+'&service_id='+service_id+'&task_type='+task_type+'&assign_status='+assign_status+'&complete_status='+complete_status+'&ref_list='+selected,
               type: "get",
               datatype: "html",
            })
            .done(function(data)
