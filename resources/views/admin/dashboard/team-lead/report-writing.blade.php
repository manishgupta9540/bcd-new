            ...
            <tbody>
                @if (count($task_report_result)>0)
                    @foreach ($task_report_result as $item)
                        ...
                        <tr>
                            <td>{{$item['service']}}</td>
                            <td><a class="report_write_link" href="{{ url('/task/assign?t_type=report_write&verify_status=all'.$user_id_url) }}">{{$item['allocated']}}</a></td>
                            <td><a class="report_write_link" href="{{ url('/task/assign?t_type=report_write&verify_status=2' .$user_id_url) }}">{{$item['completed']}}</a></td>
                            <td><a class="report_write_link" href="{{ url('/task/assign?t_type=report_write&verify_status=2&in_tat=1' .$user_id_url) }}">{{$item['completed_in']}}</a></td>
                            <td><a class="report_write_link" href="{{ url('/task/assign?t_type=report_write&verify_status=2&out_tat=1' .$user_id_url) }}">{{$item['completed_out']}}</a></td>
                            <td><a class="report_write_link" href="{{ url('/task/assign?t_type=report_write&verify_status=1' .$user_id_url) }}">{{$item['pending']}}</a></td>
                            <td><a class="report_write_link" href="{{ url('/task/assign?t_type=report_write&verify_status=1&in_tat=1' .$user_id_url) }}">{{$item['pending_in']}}</a></td>
                            <td><a class="report_write_link" href="{{ url('/task/assign?t_type=report_write&verify_status=1&out_tat=1' .$user_id_url) }}">{{$item['pending_out']}}</a></td>
                            {{-- <td>{{$item['insuff']}}</td>
