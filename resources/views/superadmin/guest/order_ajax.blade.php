                   ...
                   @if(count($items)>0)
                       @foreach ($items as $item)
                           <tr>
                               ...
                               <td>{{Helper::user_name($item->business_id)}}</td>
                               <td><i class="fas fa-rupee-sign"></i> {{$item->total_price}}</td>
                               <td>{!! Helper::get_superadmin_guest_instant_cart_service_name_slot($item->g_c_id) !!}</td>
                               <td>
                               ...
                               <td>
                                    @if($item->is_review==1)
                                        ...
                                        ?>
                                        <br>
                                        <p> <i class="fa fa-info-circle " data-toggle="tooltip" data-original-title="{{ $item->comments ? $item->comments :'N/A' }}"></i> </p>
                                    @else
                                            --
                                    @endif
                                </td>
